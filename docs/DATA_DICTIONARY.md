...
| public_updated_at | timestamp | Last significant editorial update. | Content API | Shared across HTML pages within a publication (for example, HTML publications with multiple parts). |
| publishing_app | string | CMS used to publish content (e.g. Publisher, Whitehall). | Content API | Applies to all non-attachment HTML pages. |
| is_historic | boolean | Whether content is in â€˜history modeâ€™ (associated with a previous government). | Content API | For attachments, we derive from the parent content item. |
...
| users_file_download | numeric | Users who downloaded attachments. | Google Analytics | Applies only to HTML pages. |
...
| sentence_count | numeric | Total number of sentences. | Screaming Frog | Requires successful scrape. |
| average_words_per_sentence | numeric | Average sentence length. | Screaming Frog | Requires successful scrape. |
| flesch_reading_ease_score | numeric | Readability score (0â€“100). | Screaming Frog | Requires successful scrape. |
| readability | string | Readability classification label. | Screaming Frog | Requires successful scrape. |
| is_broken | boolean | Whether the URL returns a 404. | Screaming Frog | Applies to HTML and PDF. |
| is_redirecting | boolean | Whether the URL redirects. | Screaming Frog | Applies to HTML and PDF. |
...
| Field | Format | Description | Source | Logic |
|------|--------|-------------|--------|-------|
| n_links | numeric | Total number of links on the page. | Derived | HTML pages only. |
| n_broken_links | numeric | Number of broken links. | Derived | HTML pages only. |
| n_redirecting_links | numeric | Number of redirecting links. | Derived | HTML pages only. |
| total_headings | numeric | Count of headings (H1â€“H5). | Derived | HTML pages only. |
...
| expected_reading_time_mins | numeric | Estimated reading time based on word count. | Derived | Applies where word count exists. |
...
| Field | Format | Description | Source | Logic |
|------|--------|-------------|--------|-------|
| accessible_url | string | URL of an accessible version of the content. | Derived | Applies to HTML or qualifying PDFs. |
| accessible_url_confidence_score | numeric | Confidence score for accessible match. | Derived | â‰¥0.8 for PDFs; 1 for HTML. |
| has_accessible_version | boolean | Whether an accessible version exists. | Derived | TRUE if confidence â‰¥0.9. |
